...
#
# Tags look like `v<Navidrome version>-<release>`:
#
# - if defaults/main.yml points at a Navidrome version that has never been
#   released, the release counter restarts at 0 (`v0.63.2-0`)
# - otherwise the counter is incremented (`v0.63.2-1`), but only if something
...
version="$(sed -nE 's|^navidrome_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Navidrome version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `0.63.2`), but the tags
# do (`v0.63.2-0`). Stripping any `v` before prepending one keeps this correct
